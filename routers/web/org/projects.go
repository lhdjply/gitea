...
func ViewProject(ctx *context.Context) {
	...
	var availableRepos []*attachment_model.Repository
	if project.OwnerID > 0 {
		allRepos, err := attachment_model.GetOrgRepositories(ctx, project.OwnerID)
		if err != nil {
...
func AddIssueToColumn(ctx *context.Context) {
	...
	// Get the repository
	repository, err := attachment_model.GetRepositoryByID(ctx, repoID)
	if err != nil {
		ctx.NotFoundOrServerError("GetRepositoryByID", attachment_model.IsErrRepoNotExist, err)
		return
	}
