...
func AddIssueToColumn(ctx *context.Context) {
	...
	if err := issues_model.IssueAssignOrRemoveProject(ctx, issue, ctx.Doer, []int64{project.ID}); err != nil {
...
func AddPullToColumn(ctx *context.Context) {
	...
	if err := issues_model.IssueAssignOrRemoveProject(ctx, pull, ctx.Doer, []int64{project.ID}); err != nil {
		ctx.ServerError("IssueAssignOrRemoveProject", err)
		return
	}
...
func UnbindIssueFromColumn(ctx *context.Context) {
	...
	// Remove issue from project by passing nil (all projects removed)
	if err := issues_model.IssueAssignOrRemoveProject(ctx, issue, ctx.Doer, nil); err != nil {
		ctx.ServerError("IssueAssignOrRemoveProject", err)
		return
	}
