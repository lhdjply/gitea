  ...
  if (issueRepoSelect) {
    issueRepoSelect.addEventListener('change', async () => {
      ...
      if (!repoSelect.value && !repoInput.value) {
        ...
        issueSelect.innerHTML = html`<option value="">${window.config.i18n['repo.issues.choose_issue']}</option>`;
        return;
      }
      ...
      issueSelect.disabled = true;
      issueSelect.innerHTML = html`<option value="">${window.config.i18n['loading']}</option>`;
  ...
  if (pullRepoSelect) {
    pullRepoSelect.addEventListener('change', async () => {
      ...
      if (!repoSelect.value && !repoInput.value) {
        ...
        pullSelect.innerHTML = html`<option value="">${window.config.i18n['repo.pulls.choose_pull']}</option>`;
        return;
      }
      ...
      pullSelect.disabled = true;
      pullSelect.innerHTML = html`<option value="">${window.config.i18n['loading']}</option>`;
